...
namespace TorchSharp
{
    ...
    public class TestDataLoader
    {
        ...
        private class TestDatasetFromEnumerable<T> : IDataset<T>
        {
            private readonly T[] values;
            public TestDatasetFromEnumerable(IEnumerable<T> values)
            {
        ...
        {
            using var dataset1 = new TestDatasetFromEnumerable<(int, int)>(new[] {
                (1, 1), // dataset 1 value 1
                (1, 2), // dataset 1 value 2
                (1, 3),
            });
            using var dataset2 = new TestDatasetFromEnumerable<(int, int)>(new[] {
                (2, 1),
                (2, 2),
            });
            using var dataset3 = new TestDatasetFromEnumerable<(int, int)>(new[] {
                (3, 1),
                (3, 2),
                (3, 3),
                (3, 4),
        ...
        {
            using var dataset = torch.utils.data.ConcatDataset(
                new[] {
                    new TestDataset(),
                    new TestDataset(),
                });

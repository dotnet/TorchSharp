...
4. Score each `[ci-scan]` issue against the rubric:
   ...
   - **Classification matches the failure** — build break vs test failure correct; libtorch download / push / sign failures should never have been filed (infra noise).
   - **Signature is specific** — not a bare test/method name, generic exception, exit code, or a phrase that also appears on `[PASS]`/`[SKIP]` lines of the same log.

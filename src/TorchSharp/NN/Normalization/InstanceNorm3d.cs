...
namespace TorchSharp
{
    ...
    namespace Modules
    {
        ...
        /// </summary>
        public sealed class InstanceNorm3d : InstanceNorm
        {
            internal InstanceNorm3d(long num_features, 
                                double eps, 
                                double momentum, 
                                bool affine, 
                                bool track_running_stats, 
                                Device? device, 
                                ScalarType? dtype) : base(num_features, eps, momentum, affine, track_running_stats, device, dtype, nameof(InstanceNorm3d))
            {
            }

            protected override long GetNumberOfBatchDimensions() => 4;

            protected override void ValidateInputDimensions(Tensor input)
            {
                if (input.ndim != 4 && input.ndim != 5)
                    throw new ArgumentException($"expected 4D or 4D input, but got {input.ndim}D input.");
            }
        }
    }
    ...
    public static partial class torch
    {
        public static partial class nn
        {
            ...
            /// <returns></returns>
            public static InstanceNorm3d InstanceNorm3d(long num_features, double eps = 1e-05, double momentum = 0.1, bool affine = false, bool track_running_stats = false, Device? device = null, ScalarType? dtype = null)
            {
                return new InstanceNorm3d(num_features, eps, momentum, affine, track_running_stats, device, dtype);
            }
        }
    }
}

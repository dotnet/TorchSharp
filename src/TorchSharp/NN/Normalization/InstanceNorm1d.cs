...
namespace TorchSharp
{
    ...
    namespace Modules
    {
        ...
        /// </summary>
        public sealed class InstanceNorm1d : InstanceNorm
        {
            internal InstanceNorm1d(long num_features, 
                                double eps, 
                                double momentum, 
                                bool affine, 
            ...
            protected override long GetNumberOfBatchDimensions() => 2;

            protected override void ValidateInputDimensions(Tensor input)
            {
                if (input.ndim != 2 && input.ndim != 3)
                    throw new ArgumentException($"expected 2D or 3D input, but got {input.ndim}D input.");
            }
        }
    }
    ...
    public static partial class torch
    {
        public static partial class nn
        {
            ...
            /// <returns></returns>
            public static InstanceNorm1d InstanceNorm1d(long num_features, double eps = 1e-05, double momentum = 0.1, bool affine = false, bool track_running_stats = false, Device? device = null, ScalarType? dtype = null)
            {
                return new InstanceNorm1d(num_features, eps, momentum, affine, track_running_stats, device, dtype);
            }
        }
    }
}

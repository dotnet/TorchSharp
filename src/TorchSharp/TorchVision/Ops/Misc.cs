...
namespace TorchSharp
{
    public static partial class torchvision
    {
        public static partial class ops
        {
            ...
            /// <param name="norm_layer">Norm layer that will be stacked on top of the convolution layer. If ``null``, ``torch.nn.BatchNorm2d`` is used.</param>
            /// <param name="activation_layer">Activation function which will be stacked on top of the normalization layer (if not None), otherwise on top of the conv layer. If ``null``, ``torch.nn.ReLU`` is used.</param>
            ...
            /// <param name="bias">Whether to use bias in the convolution layer. By default, is false.</param>
                ...
                bool? bias = null)
            {
                ...
                if (norm_layer == null) {
                    layers.Add(torch.nn.BatchNorm2d(out_channels));
                } else {
                    layers.Add(norm_layer(out_channels));
                }

                if (activation_layer == null) {
                    layers.Add(torch.nn.ReLU(inplace));
                } else {
            ...
            /// <param name="padding">Padding added to all four sides of the input. Default: null, in which case it will calculated as ``padding = (kernel_size - 1) // 2 * dilation``</param>
            /// <param name="groups">Number of blocked connections from input channels to output channels.</param>
            /// <param name="norm_layer">Norm layer that will be stacked on top of the convolution layer. If ``null``, ``torch.nn.BatchNorm3d`` is used.</param>
            /// <param name="activation_layer">Activation function which will be stacked on top of the normalization layer (if not None), otherwise on top of the conv layer. If ``null``, ``torch.nn.ReLU`` is used.</param>
            /// <param name="dilation">Spacing between kernel elements.</param>
            /// <param name="inplace">Parameter for the activation layer, which can optionally do the operation in-place.</param>
            /// <param name="bias">Whether to use bias in the convolution layer. By default, is false.</param>
                ...
                bool? bias = null)
            {
                ...
                if (norm_layer == null) {
                    layers.Add(torch.nn.BatchNorm3d(out_channels));
                } else {
                    layers.Add(norm_layer(out_channels));
                }
